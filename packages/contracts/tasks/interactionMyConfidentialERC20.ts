...
task("totalSupply").setAction(async function (
  taskArguments: TaskArguments,
  hre: HardhatRuntimeEnvironment
) {
  try {
  ...
  .setAction(async function (
    ...
    try {
      ...
      if (balanceHandle === 0n) {
        ...
        balance = await reencryptEuint64(
          wallet,
          fhevm,
          balanceHandle,
          ERC20.address
...
task("transfer")
  .addParam(
    "privatekey",
    "Private key of the address from which we are sending the tokens"
  )
  .addParam("to", "Recipient address")
